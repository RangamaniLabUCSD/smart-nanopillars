...
from mech_parser_args import add_preprocess_mech_mesh_arguments, Shape, shape2symfraction
...
def main(mesh_folder: str, shape: Shape, hEdge=0.6, hInnerEdge=0.6):
    ...
    if sym_fraction == 0:
        cell_mesh, facet_markers, cell_markers, curv_markers = mesh_gen.create_2Dcell(outerExpr = outerExpr13, innerExpr = innerExpr13,
                                                                    hEdge=hEdge, hInnerEdge=hInnerEdge,
                                                                    half_cell=True, return_curvature=True)
    else:
        cell_mesh, facet_markers, cell_markers = mesh_gen.create_3dcell(outerExpr=outerExpr13, innerExpr=innerExpr13,
                                                                        hEdge=hEdge, hInnerEdge=hInnerEdge,
        ...
        curv_markers = mesh_tools.compute_curvature(cell_mesh, facet_markers, cell_markers, [10,12], [1,2])
