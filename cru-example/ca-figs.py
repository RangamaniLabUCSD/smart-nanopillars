...
if True:#"npy-files" not in os.listdir(cur_dir):
    ...
    for i in [4,5,6]:#range(len(tests)):
        ...
        np.save(npy_dir / f"{tests[i]}-results-fixedSERCA.npy", results_all)
...
f, (ax1, ax2) = plt.subplots(2, 1, sharex=False)
plot_cru1 = "Ca"
results_cru_withSERCA = np.load(f"{cur_dir}/npy-files-new/cru-center-results-fixedSERCA.npy")
results_cru_noSERCA = np.load(f"{cur_dir}/npy-files-new/cru-center-results-noSERCA.npy")
