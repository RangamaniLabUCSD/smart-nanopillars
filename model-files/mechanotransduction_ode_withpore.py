...
def mechanotransduction_ode_calc(timeSpan, stiffness, geoParam, poreParam):
    ...
    param = [
        ...
        poreParam[0], # koutpore
        poreParam[1], # kinpore/koutpore
        poreParam[2], # porerate
        poreParam[3], # SApore
    ]
    ...
    def f(y, t, p):
        ...
        koutpore = p[103]
        kinpore = koutpore*p[104]
        porerate = p[105]
        pore = (1 - np.exp(-t/porerate))
        SApore = p[106]
        J_Ypore = pore*(SApore/Size_NM)*(kinpore*YAPTAZN - koutpore*YAPTAZnuc)
        Ytot = 1.4785e6
        YAPTAZnucphos = (Ytot - (YAPTAZN+YAPTAZP)*602.2*Size_Cyto - YAPTAZnuc*602.2*Size_Nuc)/(Size_Nuc*602.2)
        J_Yphospore = pore*(SApore/Size_NM)*(kinpore*YAPTAZP - koutpore*YAPTAZnucphos)
        if YAPTAZnucphos < 0:
            print(YAPTAZnuc)
        ...
        dydt = [
            ...
            (UnitFactor_uM_um3_molecules_neg_1 * KFlux_NM_Nuc * (J_r16 + J_Ypore)),    # rate for YAPTAZnuc
            ((UnitFactor_uM_um3_molecules_neg_1 * KFlux_PM_Cyto * J_r12) - J_r0 + SAV*KFlux_PM_Cyto*J_r2),    # rate for Fakp
            - J_r13 - (UnitFactor_uM_um3_molecules_neg_1 * KFlux_NM_Cyto * J_Yphospore),    # rate for YAPTAZP
            (J_r13 - (UnitFactor_uM_um3_molecules_neg_1 * KFlux_NM_Cyto * (J_r16 + J_Ypore))),    # rate for YAPTAZN
    ...
    #
    num_points = int((timeSpan[1]-timeSpan[0]/10)) + 1
    t = np.linspace(timeSpan[0], timeSpan[1], num_points)
