...
try: # run as python script on cluster
    ...
    results_folder = pathlib.Path("/root/scratch/smart-comp-sci-data/dendritic_spine/results1")
    mesh_file = (here / ".." / "meshes" / "1spine_PM10_PSD11_ERM12_cyto1_ER2.xml")
    new_mesh = pathlib.Path("mesh/1spine.h5")
    main_run.run_preprocess_spine_mesh(
...
geo = mesh_tools.load_mesh(filename=args["mesh_file"], 
...
uvec = model_cur.cc[sp.compartment_name].u["u"].vector()
indices = sp.dof_map
values  = uvec.get_local()
mesh_ref = parent_mesh.dolfin_mesh
funcSpace = sp.V
...
for facet in d.facets(mesh_ref):
    ...
    cur_marker = subdomain0.array()[facet_idx]
    for vertex in d.vertices(facet):
        ...
        if len(local_idx[0])==0:
            ...
            if cur_marker==10:
                values[sp.dof_map[cur_sub_idx]] = 0

for tag in ["u", "n"]:
    vec = model_cur.cc[sp.compartment_name].u[tag].vector()
...
int_vol = d.assemble(1.0*dx_int(1))
gradVec = np.array([d.assemble(d.grad(model_cur.sc["Ca"].u["u"])[2]*dx_int(1))/int_vol])
tvec = np.array([0.0])
# Solve
displayed = False
while True:
    ...
    int_val_grad = d.assemble(d.grad(model_cur.sc["Ca"].u["u"])[2]*dx_int(1))
...
# Plot the average calcium over time.
