...
def plot_time_step_vs_error(
    ...
    fig, ax = plt.subplots(1, len(diffusions), sharex=True, figsize=(10, 10))
    ...
    for j, diffusion in enumerate(diffusions):
        ...
        for k, refinement in enumerate(refinements):
            ...
            if refinement == 0:
                ...
                # breakpoint()
                ax[j].loglog(dts, (dts ** 2) / dts[-3] * results[0].l2, "k--")

        ax[j].legend()
        # ax_t[j].legend()
        ax[j].set_xlabel("Time step [s]")
    ...
    ax[0].set_ylabel(r"$ \| u_e - u \|_2$")
    fig.savefig(
        output_folder / f"timestep_vs_error.{format}",
        dpi=300,
    )
    fig_t.savefig(
...
def plot_refinement_vs_error(
    ...
    for j, diffusion in enumerate(diffusions):
        ...
        for k, dt in enumerate(time_steps):
            ...
            # dts = np.array([d.dt for d in results])
            # hmins = np.array([d.hmin for d in results])
            # hmaxs = np.array([d.hmax for d in results])
            refs = np.array([2 ** (-(d.refinement + 1)) for d in results])
        ...
        ax[j].legend()
        # ax_t[j].legend()
        ax[j].set_xlabel("$2^{-(refinement + 1)}$")
        ax_t[j].set_xlabel("Time step [s]")
        ax_t[j].set_ylabel("Total run time [s]")
