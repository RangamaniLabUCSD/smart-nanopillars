...
# * MRTF: free MRTF in the cytosol
# * MRTF_bound: MRTF bound to GActin in the cytosol
#
...
# * MRTF_nuc: concentration of MRTF in the nucleus
...
# sys.path.insert(0, "/root/shared/gitrepos/smart-comp-sci/utils")
# import spread_cell_mesh_generation as mesh_gen
# cell_mesh, facet_markers, cell_markers, substrate_markers, curv_markers = mesh_gen.create_3dcell(contactRad=20.25,
#                                                                 hEdge=0.3, hInnerEdge=0.6,
#                                                                 thetaExpr="",
#                                                                 nanopillars=[0.1, 1.0, 5.0],
#                                                                 return_curvature = True)

# substrate_markers = d.MeshFunction("size_t", cell_mesh, 2)
# for f in d.facets(cell_mesh):
#     topology, cellIndices = mesh_tools.facet_topology(f, cell_markers)
#     if topology == "boundary" and (facet_markers.array()[f.index()] != 10 or 
#                                     np.isclose(f.midpoint().z(),0)):
#         substrate_markers.set_value(f.index(), 10)

mesh_folder = pathlib.Path("/root/shared/gitrepos/smart-comp-sci-data/meshes/nanopillars/nanopillars_h1.0_p5.0_r0.5_cellRad20.01")
results_folder = pathlib.Path("results")
# mesh_tools.write_mesh(cell_mesh, facet_markers, cell_markers, str(mesh_folder / "spreadCell_mesh.h5"))
...
try: # run as python script on cluster
    ...
    #     num_refinements=0,
    #     full_3d=False,
    #     nanopillar_radius=0.5,
    #     nanopillar_height=2.0,
    #     nanopillar_spacing=2.0,)
    args = {}
...
if args["reaction_rate_on_np"] != 1.0:
...
if well_mixed:
    ...
    # MRTF = Species(
    #     "MRTF", 0.0, vol_unit, D_mixed, D_unit, "Cyto"
    # )  # free MRTF in the cytosol, very rough estimate for diffusion
    # MRTF_bound = Species(
    #     "MRTF_bound", 0.65, vol_unit, D_mixed, D_unit, "Cyto"
    # )  # free MRTF in the cytosol, very rough estimate for diffusion
    # MRTF_nuc = Species("MRTF_nuc", 0.301, vol_unit, D_mixed, D_unit, "Nuc")
else:
    ...
    # MRTF = Species(
    #     "MRTF", 0.0, vol_unit, 5.0, D_unit, "Cyto"
    # )  # free MRTF in the cytosol, very rough estimate for diffusion
    # MRTF_bound = Species(
    #     "MRTF_bound", 0.65, vol_unit, 5.0, D_unit, "Cyto"
    # )  # free MRTF in the cytosol, very rough estimate for diffusion
...
geo = mesh_tools.load_mesh(filename=pathlib.Path(args["mesh_folder"]) / "spreadCell_mesh.h5",
                           extra_keys=["subdomain0_2"])
substrate_markers = geo.subdomains[0]
substrate_A.restrict_to_subdomain(substrate_markers, 10)
NMmesh = d.create_meshview(geo.mf_facet, 12)
NMArea = d.assemble(1.0*d.Measure("dx", NMmesh))*8
refArea = 365.9#(4*np.pi*(((5.3*5.3)**1.6 + (5.3*2.4)**1.6 + (2.4*5.3)**1.6)/3)**(1/1.6))/.8**2
LaminA_tot.value = LaminA_tot.value*(refArea/NMArea)
NPC_tot.value = NPC_tot.value*(refArea/NMArea)
# -
